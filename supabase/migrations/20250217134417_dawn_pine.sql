...
CREATE OR REPLACE FUNCTION generate_product_slug(title text)
RETURNS text AS $$
DECLARE
  slug text;
  base_slug text;
...
BEGIN
  ...
  WHILE EXISTS (SELECT 1 FROM courses WHERE slug = slug) LOOP
    counter := counter + 1;
    slug := base_slug || '-' || counter::text;
  END LOOP;
...
CREATE TRIGGER set_product_slug
  ...
  FOR EACH ROW
  EXECUTE FUNCTION generate_product_slug(NEW.title);

CREATE TRIGGER set_course_slug
  BEFORE INSERT ON courses
  FOR EACH ROW
  EXECUTE FUNCTION generate_course_slug(NEW.title);
